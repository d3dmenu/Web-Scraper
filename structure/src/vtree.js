...
class VTree {
  constructor(container) {
    ...
    this.d3.zoomListener = d3.behavior
      .zoom()
      .scaleExtent([0.1, 10])
      .on("zoom", () => {
        const e = d3.event;
  ...
  width(width) {
    ...
    this.d3.container.select("svg").attr("width", width);
  ...
  height(height) {
    ...
    this.d3.container.select("svg").attr("height", height);
  ...
  _debugDrawNodeInfo(node) {
    ...
    // x, y
    const xy = node.g.append("text").text(`x=${node.x} y=${node.y}`);
